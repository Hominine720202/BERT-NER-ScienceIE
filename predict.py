...
parser.add_argument('--logit', default=1, help="A threshold value")
...
def predict(line, model, data_loader, params): # added by temco for prediction
    ...
    pred_tags = []
    pred_tags.extend([idx2tag.get(idx) for indices in np.argmax(batch_output, axis=2) for idx in indices])
    f=open("prediction/tags.txt","a",encoding="utf-8")
    for chac in pred_tags:
        f.write(chac+" ")
    f.write("\n")
    f.close()
    f=open("prediction/sentences.txt","a",encoding="utf-8")
    f.write(line+"\n")
    f.close()
...
if __name__ == '__main__':
    ...
    sentences = []
    file = open('data/owndata/test/sentences.txt')
    for line in file.readlines():
        sentences.append(line)
    file.close()
    f=open("prediction/tags.txt","w",encoding="utf-8")
    f.truncate()
    f.close()
    f=open("prediction/sentences.txt","w",encoding="utf-8")
    f.truncate()
    f.close()
    for sentence in sentences:
        predict(sentence,model,data_loader,params)
